...
async def test_pipeline_phases_called_in_order():
    ...
    assert h2_lines[:2] == ["## 先说结论", "## 目录"]
    assert "- 技术健康度与搜索表现" in result
...
def test_table_of_contents_uses_locale_labels():
    from opencmo.report_pipeline import _build_table_of_contents

    toc = _build_table_of_contents(
        [({"title": "Search Visibility"}, "## Search Visibility\n\nBody")],
        locale="en",
    )

    assert toc.splitlines()[:4] == [
        "## Table of Contents",
        "",
        "- Charts at a Glance",
        "- Context and Decision Criteria",
    ]
    assert "- Strategic Recommendations and Action Roadmap" in toc


def test_pipeline_dimensions_include_periodic_history_keys():
    from opencmo.report_pipeline import _DIMENSIONS, _slice_dimension

    facts = {
        "seo_history": [{"score_performance": 0.7}],
        "geo_history": [{"geo_score": 55}],
        "community_history": [{"total_hits": 4}],
    }
    dimensions = {dimension["id"]: dimension for dimension in _DIMENSIONS}

    assert _slice_dimension(facts, dimensions["seo_tech"])["seo_history"] == facts["seo_history"]
    assert _slice_dimension(facts, dimensions["ai_visibility"])["geo_history"] == facts["geo_history"]
    assert _slice_dimension(facts, dimensions["community_market"])["community_history"] == facts["community_history"]
